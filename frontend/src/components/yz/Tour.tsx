 ...
 *
 * The dim layer is built from four rectangles around the target rather
 * than one full-screen overlay, so the target stays clickable through
 * the spotlight hole. That's what makes "action" work without any
 * pointer-events tricks.
 *
 * Missing target (route changed, conditional rendering) → step is
 * skipped gracefully rather than showing an empty halo.
 */
...
const PAD = 8; // halo padding around the target in px
const TIP_W = 320;
...
export function Tour({
  ...
  // Reset to step 0 when the tour re-opens.
  useEffect(() => {
    if (open) setIdx(0);
  }, [open]);
  ...
  // window resize / scroll. useLayoutEffect to avoid a one-frame
  // flash where the spotlight is at the wrong position.
  useLayoutEffect(() => {
    if (!open || !step) return;
    const measure = () => {
      ...
      // Bring the target into view first — otherwise a step anchored
      // to something below the fold is invisible behind the dim layer.
  ...
  // Action mode: listen for clicks anywhere on the page; if the click
  // happened inside the target subtree, advance. Capture phase so we
  // see the click even if the target's own handler stops propagation.
  useEffect(() => {
    if (!open || !step) return;
    ...
    const onClick = (e: MouseEvent) => {
      ...
      if (!target) return;
      const matched = target.closest(step.targetSelector);
      if (!matched) return;
      // Defer one tick — let the target's own click handler run (open
      // a sheet, copy a link, etc.) before we tear down the step and
      // potentially blow away the new DOM node.
      setTimeout(() => {
  ...
  // Stale selector — auto-advance so we don't strand the user.
  if (rect === null) {
    queueMicrotask(() => {
      if (idx + 1 < steps.length) setIdx(idx + 1);
      else onClose();
  ...
  // Pulsing pointer position — sits at the centre of the target,
  // slightly above so it visually "drops down" with the bounce.
  ...
  return (
    <div className="fixed inset-0 z-[3000]" aria-live="polite">
      {/* Dim layer — four rectangles around the target, leaving the
          spotlight area clickable. */}
      <div
      ...
      {/* Halo border with a soft pulse so the target visibly says
          "I'm the thing you should look at". */}
      <div
      ...
      {/* Action-mode pointer: a finger icon that bounces over the
          target. Pure CSS animation (defined in globals via the
          yz-tour-bounce keyframes inline below) so we don't need an
      ...
      {/* Inline keyframe styles. Cheaper than a separate CSS file for
          two effects only the tour uses. */}
      <style jsx>{`
