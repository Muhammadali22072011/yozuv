...
class Business(Base):
    ...
    # Partner (B2B) referral: this business's own shareable code, who
    # referred it to the platform, and a discount it has earned (and not
    # yet spent) on its next subscription payment.
    partner_code: Mapped[str | None] = mapped_column(String(16), unique=True, nullable=True)
    referred_by_business_id: Mapped[uuid.UUID | None] = mapped_column(
        UUID(as_uuid=True), ForeignKey("businesses.id", ondelete="SET NULL"), nullable=True
    )
    pending_partner_discount_percent: Mapped[int] = mapped_column(Integer, default=0, nullable=False)
